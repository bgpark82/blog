...
PubSub 방식의 비동기 프로세스는 주로 BlockingQueue를 사용한다. Publisher가 put을 메소드 사용 시 Qeue가 꽉차있다면 대기하게 되고 Subscriber가 Queue의 값을 take할 때 Queue가 없다면 올때까지 스레드가 대기하게 됩니다. 자바는 Executor라는 인터페이스로 Thread Pool을 구현하게되는데 이때 내부적으로 BlockingQueue를 사용하게 됩니다. 
...
1. ApplicationEventPublisher 구조 설명 (커스텀 Publisher/Subscriber의 문제점 해결책)
2. 요구사항 1, 2 해결 (커스텀 Publisher/Subscriber의 문제점 개선점)
3. 개선 사항, 아쉬운점
